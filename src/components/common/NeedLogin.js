    ...
    return (
        <div className={addClass ? 'relative w-full h-full ' + addClass : 'relative w-full h-full '}>
            <div className={classNames("flex flex-col items-center justify-center text-txtSecondary dark:text-txtSecondary-dark", {
                ...
                <a href={getLoginUrl('sso', 'login')} className={classNames("mt-4 rounded-md bg-dominant text-white text-sm px-4 py-2", { '!text-white !bg-namiapp-green-1 w-full h-12 flex items-center justify-center': isNamiapp })}>
                    {_message}
                </a>
            </div>
        </div>
