...
const NavBar = () => {
    ...
    const _renderMenuHaveSubMenu = (item, key) => (
        <Popover className="relative" key={key}>
            {({ open }) => (
                <>
                    <Popover.Button key={`Popover:${item.name}`} type="button" className="inline-flex items-center focus:outline-none" aria-expanded="false">
                    ...
                    >
                        <Popover.Panel static className="absolute z-10 mt-3 transform w-screen max-w-[18rem] rounded-md bg-white shadow-xl">
                                    ...
                                    sub.disabled ? (
                                        <div key={iSub + sub?.name} className="relative grid gap-6 bg-white sm:gap-8">
                                            <a className="navbar-item-link group disabled">
                                                <span className="text-black-500">{sub.icon}</span>
                                                <div className="ml-3">
                                                    <div className="flex items-center">
                                                        <span className="text-sm font-medium text-black-400">{t(`submenu.${sub.name}`)}</span>
                                                        <span className="label label-red ml-3">{t('coming_soon')}</span>
                                                    </div>
                                                    <div className="mt-1 text-xs text-gray-500">{t(`submenu.${sub.name}_description`)}</div>
                                                </div>
                                            </a>
                                        </div>
                                    ) : (
                                        <div key={iSub + sub?.name} className="relative grid gap-6 sm:gap-8">
                                            <Link href={sub.route} locale={locale} prefetch={false}>
                                                <a className="navbar-item-link group ">
                                                    <span className="text-black-500 group-hover:text-violet-700">{sub.icon}</span>
                                                    <div className="ml-3">
                                                        <div className="flex items-center">
                                                            <span className="text-sm font-medium group-hover:text-violet-700">{t(`submenu.${sub.name}`)}</span>
                                                        </div>
                                                        <div className="mt-1 text-xs text-gray-500">{t(`submenu.${sub.name}_description`)}</div>
                                                    </div>
                                                </a>
    ...
    const _renderMenu = (item, key) => {
        if (item.disabled) {
            return (
                <div className="flex items-center cursor-not-allowed" key={key}>
                    <p className="navbar-item text-black-400">{t(`menu.${item.name}`)}</p>
                    <span className="label label-red ml-3">{t('coming_soon')}</span>
                </div>
            );
        }
        return (
            <Link href={item.route} locale={locale} key={key} prefetch={false}>
                <a className="navbar-item">{t(`menu.${item.name}`)}</a>
            </Link>
        );
    };
        ...
        <>
            <header className="relative bg-white">
                        ...
                        {user && !loadingUser && (
                            <div className="hidden md:flex items-center justify-end md:flex-1 lg:w-0">
                                ...
                                <Popover className="relative">
                                    <Popover.Button type="button" className="inline-flex items-center focus:outline-none h-11" aria-expanded="false">
                                        <span className="mr-3">
                                            <img src={urlAvatar()} width={28} height={28} className="rounded-full w-7 h-7 min-w-[28px]" />
                                        </span>
                                        <span className="text-sm font-semibold">{user?.username || (user?.email || 'Nami User').substring(0, 30)}</span>
                                    </Popover.Button>
                                    <Popover.Panel
                                        // static
                                        ...
                                        <div className="overflow-hidden">
                                            <div>
                                                <Link href="/my/account/profile" locale={locale} prefetch={false}>
                                                    <div className="px-2 py-2 flex items-center hover:bg-teal-5 font-medium cursor-pointer group">
                                                        <span className="text-black group-hover:text-teal">
                                                            <IconProfile size={16} />
                                                        </span>
                                                        <span className="ml-3 flex-grow text-black-700 group-hover:text-teal">{t('account')}</span>
                                                    </div>
                                                </Link>
                                            </div>
                                            <div>
                                            ...
                                            <div className="border-0 border-t border-black-200 font-medium">
                                                ...
                                                >
                                                    ...
                                                    <span className="ml-3 text-black-700 group-hover:text-teal">{t('logout')}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </Popover.Panel>
                ...
                <Transition show={open} as={Fragment}>
                    <Dialog as="div" className="fixed inset-0 z-10 overflow-y-auto" initialFocus={cancelButtonRef} static open={open} onClose={closeModal}>
                        <div className="min-h-screen text-right">
                            ...
                            >
                                <div className=" w-3/4 inline-block overflow-hidden text-left align-middle bg-white shadow-xl h-screen relative">
                                    <Dialog.Title className="px-5 mt-4">
                                        <div className="flex justify-between items-center">
                                            <div className="text-xl font-medium leading-8 text-black-800" />
                                    ...
                                    <div className="px-5">
                                        ...
                                        {!user && !loadingUser && (
                                            <div className="mt-[100px]">
                                                <div>
                                                    <button className="btn btn-primary w-full" type="button" onClick={openModalRegister}>
                                                        {t('common:sign_up')}
                                                    </button>
                                                </div>
                                                <div className="text-xs text-center mt-3">
                                                    <span className="text-black-500 ">Bạn đã có tài khoản?&nbsp;</span>
                                                    <span className="text-teal font-bold" onClick={openModalRegister}>
                                                        Đăng nhập
                                                    </span>
                                                </div>
                                            </div>
                ...
                <Transition show={openRegister} as={Fragment}>
                    ...
                    >
                        <div className="md:min-h-screen min-h-[calc(100%-10rem)] px-4 text-center">
                            ...
                            >
                                <div className="inline-block w-full mb-8 overflow-hidden text-left align-middle transition-all transform  shadow-xl ">
                                    <Dialog.Title className="">
                                        <div className="flex justify-between items-center">
                                            <div className="text-xl font-medium leading-8 text-black-800" />
                                            <button className="btn btn-icon" type="button" onClick={closeModalRegister}>
                                        ...
                                        <div className="px-6 py-8 text-center !font-bold">
                                            ...
                                            <div className="">
                                                <a href={DOWNLOAD_APP_LINK.IOS} target="_blank" rel="noreferrer">
                                                    <button className="btn btn-black w-full mb-2" type="button" rel="noreferrer">
