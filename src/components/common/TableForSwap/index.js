...
const ReTable = memo(({ data, columns, loading, resizable, sort, pagination, paginationProps, tableStyle, tableStatus, useRowHover, ...restProps }) => {
    // * Init State
            ...
            columns.forEach((c) => {
                ...
                if (castArray(sort).includes(c.key)) {
                    ...
                    item = {
                        ...c,
                        title: (
                            <div className={className.join(' ')} onClick={() => !c?.preventSort && setSorter({ [`${c.key}`]: !sorter?.[`${c.key}`] })}>
    ...
    return (
        <ReTableWrapper isDark={currentTheme === THEME_MODE.DARK} useRowHover={useRowHover} {...tableStyle}>
            {renderTable()}
            {renderPagination()}
        </ReTableWrapper>
    );
...
const ReTableWrapper = styled.div`
  ...
  .rc-table tbody tr td, .rc-table tbody tr th {
    background: ${({ isDark }) => (isDark ? colors.namiapp.black.DEFAULT : colors.white)};
  }
  ...
  table {
    ...
    tbody tr {
      .rc-table-cell-fix-right:last-child:not(.rc-table-cell-fix-sticky) {
        background: ${({ isDark }) => (isDark ? colors.namiapp.black.DEFAULT : colors.white)} !important;
      }
    }
