...
export default (props) => (WrappedComponent) => {
    return (wrappedProps) => {
        ...
        return (
            <>
                <MaldivesLayout contentWrapperStyle={{ ...wrapperStyle, position: useModal ? 'unset !important' : 'relative' }} hideInApp={hideInApp && isApp}>
                    <Background isDark={currentTheme === THEME_MODE.DARK}>
                        <CustomContainer className={`mal-container h-full ${props?.containerClassname ? props.containerClassname : ''}`}>
                            <div
                                className={
                                    hideInApp && isApp
