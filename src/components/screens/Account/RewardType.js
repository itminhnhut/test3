...
import { formatNumber, formatTime, getLoginUrl, getS3Url, formatCurrency } from 'redux/actions/utils';
...
const RewardType = memo(({ data, active, assetConfig, claim, claiming, onClaim }) => {
                ...
                return (
                    <div style={width >= BREAK_POINTS.lg ? { width: 'calc(100% - 250px)' } : undefined}
                        ...
                        <div className="w-full mt-1.5 text-xs lg:text-sm">
                            ({t('common:end_at')} {formatTime(data?.ended_at, 'HH:mm dd-MM-yyyy')})
                        </div>
                    </div>
                )
                    ...
                    <div className="my-4 w-full xl:min-w-[40%] font-medium">
                        <div className="flex items-center">
                            ...
                            <div className="flex text-right text-xs md:text-sm xl:text-[16px] xl:pr-0 ml-2">
                                ...
                                <span>/</span>
                                <span className="break-normal">{formatCurrency(target, 3)}</span>
                            </div>&nbsp;
                            <div className="text-xs md:text-sm xl:text-[16px] whitespace-nowrap	">({(data?.user_metadata?.reward?.num_of_rewards ?? 0) + ' ' + data?.reward?.unit[language]})</div>
                        </div>
    ...
    }, [data?.condition, data?.metadata, data?.user_metadata, auth, reachedProgress, language, isFirst, width, assetConfig])
    ...
    const renderButtonGroup = useCallback(() => {
        ...
        !isExpired && !isClaimed && data?.cta_button_url?.forEach((action, index) => {
            ...
            if (action?.type === Types.TASK_CTA_TYPE.URL) {
                buttonGroup.push(
                    <RewardButton key={`reward_button_${index}`} title={rewardProps?.title}
        ...
        return (
            <div className="flex items-center lg:justify-end ">
                {!auth ?
                    <RewardButton title={t('common:sign_in')} buttonStyles="mt-3 w-full min-w-[90px] sm:max-w-[120px]"
                        componentType="a" href={getLoginUrl('sso', 'login')}
