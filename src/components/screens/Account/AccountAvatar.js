...
import { API_GET_USER_AVATAR_PRESET, SET_USER_AVATAR, USER_AVATAR_PRESET } from 'redux/actions/apis';
...
export const UploadAvatar = ({ t, onDropCustomAvatar, className = '' }) => {
        ...
        >
            {({ getRootProps, getInputProps }) => (
                ...
                >
                    ...
                    <span className="text-sm break-all text-txtSecondary dark:text-txtSecondary-dark">{t('profile:support_image_type')}</span>
                    {avatarIssues && (
...
const AccountAvatar = ({ currentAvatar }) => {
    ...
    useEffect(() => {
        const { DONE, FAILURE, IDLE } = UPLOADING_STATUS;
        let timeoutId;
    ...
    useEffect(() => {
        axios
            .get(API_GET_USER_AVATAR_PRESET, {
                params: {
                    skip: 0,
                    limit: 999
                }
    ...
    return (
        <>
            <div className="relative w-[6.5rem] h-[6.5rem] md:w-[8.75rem] md:h-[8.75rem] -mt-14">
                <div className="w-full h-full rounded-full border-4 border-white dark:border-dark overflow-hidden">
                    {uploadStatus === UPLOADING_STATUS.UPLOADING ? (
                        <div className="flex items-center justify-center bg-white dark:bg-darkBlue-3 w-full h-full rounded-full">
            ...
            <ModalV2 isVisible={openConfirmModal} onBackdropCb={cancelChangeAvatar} className="md:w-[30rem]" wrapClassName="text-center" isMobile={isMobile}>
                <img width={124} height={124} className="mx-auto mt-6 mb-3 rounded-full object-cover h-[124px]" src={avatar?.src} alt="Nami Exchange" />
            ...
            >
                <div className="flex flex-col flex-1 min-h-0">
                    ...
                    <div className="flex flex-col flex-col-reverse md:flex-row md:pb-10 flex-1 min-h-0">
                        <div
                            ref={avatarRef}
                        ...
                        >
                                        ...
                                        <div
                                            ...
                                            className={classnames(
                                                ...
                                                'transition duration-100',
                                                {
                                                    ...
                                                    'border-divider text-txtSecondary dark:text-txtSecondary-dark dark:border-divider-dark':
                            ...
                            <div className="space-y-6">
                                {avatarSets.map((avatarSet) => {
                                    return (
                                        <div id={`category_${avatarSet.category.id}`} data-id={avatarSet.category.id} key={avatarSet.category.id}>
                                                ...
                                                <div className="flex flex-wrap gap-x-6 gap-y-4">
                                                    {avatarSet.images.map((image, index) => {
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                <img className="rounded-full" width={68} height={68} src={image} alt="Nami Exchange" />
                                                            </div>
                                                        );
                                                    })}
                                                </div>
                    ...
                    <div className="pt-4 md:pt-0 border-t md:border-none border-divider dark:border-divider-dark">
                        <ButtonV2 disabled={!avatar} onClick={() => setOpenConfirmModal(true)}>
                            {t('common:confirm')}
                        </ButtonV2>
                    </div>
                </div>
