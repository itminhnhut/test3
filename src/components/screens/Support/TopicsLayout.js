    ...
    const renderTopics = useMemo(
        () =>
            topics?.map((item) => (
                <div key={item?.id}>
                    <div
                        className={classNames('h-14 mb-3 flex items-center hover:bg-hover dark:hover:bg-hover-dark cursor-pointer', {
                        ...
                        >
                            ...
                            >
                                <div className="h-6 w-6 mr-3 sm:mr-6">
                                    <Image src={getS3Url(getSupportCategoryIcons(item?.id))} layout="responsive" width={24} height={24} />
                                </div>
                            ...
                            <span
                                ...
                                onClick={() =>
                                    setShowDropdown((prevState) => ({
                                        ...prevState,
                                        [item?.displaySlug]: !showDropdown?.[item?.displaySlug]
                                    }))
                                }
                            >
                                ...
                                >
                                    <a
                                        className={classNames(
                                            ...
                                            {
                                                '!font-semibold !text-txtPrimary dark:!text-gray-4 leading-6': subCats?.displaySlug === faqCurrentGroup
                                            }
                                        )}
                                    >
                                        {subCats?.title}
            ...
            <div className="bg-bgPrimary dark:bg-dark pt-0">
                ...
                <div className="bg-bgPrimary dark:bg-dark rounded-t-[20px] lg:mt-0">
                    <div className="block sm:flex min-h-[500px] relative 2xl:max-w-screen-xxl m-auto">
                        <div style={{ width: COL_WIDTH, minWidth: COL_WIDTH }} className="pt-16 sm:py-20 border-r border-divider dark:border-divider-dark">
                            ...
                            {isMobile ? (
                                ...
                                >
                                    <div className="w-6 h-6 mr-3">
                                        <Image src={getS3Url(getSupportCategoryIcons(mainTopic?.id))} width={24} height={24} />
                                    </div>
                                    <div className="flex justify-between w-full" onClick={() => setToggleMenu(!toggleMenu)}>
                                        <div className="flex-grow text-txtPrimary dark:text-gray-4 font-semibold text-sm">
                                            {' '}
                                            {subTopics ? subTopics?.title : mainTopic?.title}
                                        </div>
                                        <div>
                                            {toggleMenu ? (
                                                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            >
                                <div className="bg-bgPrimary dark:bg-dark pt-3 sm:pt-0">{renderTopics}</div>
                                <div
                                    className="h-[calc(100%-600px)] min-h-[150px] w-full bg-black-800/[.6] dark:bg-black-800/80 sm:hidden"
                        ...
                        <div className="w-full">
                            <div className="flex-grow w-full px-4 py-4 pb-12 sm:px-[112px] sm:py-20">{children}</div>

                            {lastedArticles && Array.isArray(lastedArticles) && lastedArticles?.length && (
                                <div className="px-4 sm:px-[112px] pb-24">
                                    <div className="text-txtPrimary dark:text-gray-4 text-2xl font-semibold mb-6 sm:mb-2">
                                        {t('support-center:lasted_articles')}
                                    </div>
                                    <LastedArticles
                                        isMobile={isMobile}
                                        lastedArticles={lastedArticles}
                                        language={language}
                                        isApp={isApp}
                                        t={t}
                                        containerClassName="!bg-transparent px-0 !shadow-none"
                                    />
                                </div>
                            )}
                        </div>
