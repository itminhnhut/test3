...
const getPairConfig = createSelector(
    [(state) => state?.futures?.pairConfigs, (utils, params) => params],
    (pairConfigs, params) => {
...
const calLiqPrice = (side, quantity, open_price, margin, fee) => {
    ...
    return (
        (size * open_price + fee - margin) /
        (quantity * (number - DefaultFuturesFee.NamiFrameOnus))
    );
};
const AdjustPositionMargin = ({
    order,
    pairPrice,
    onClose,
    forceFetchOrder,
    ...
    const pairConfig = useSelector((state) =>
        getPairConfig(state, { symbol: order?.symbol })
    );
    const assetConfig = useSelector((state) =>
        getAsset(state, { id: pairConfig?.quoteAssetId })
    );

    const { t } = useTranslation();

    const lastPrice =
        order?.side === VndcFutureOrderType.Side.BUY
            ? pairPrice?.bid
            : pairPrice?.ask;
    ...
    const {
        ...
        let maxRemovable =
            order.margin - initMargin * minMarginRatio + Math.min(profit, 0);
        if (!minMarginRatio) {
            maxRemovable = 0;
        }
        ...
        const newMargin =
            +order?.margin +
            (adjustType === ADJUST_TYPE.REMOVE ? -amount : +amount);
        return {
            newMargin,
            newLiqPrice: calLiqPrice(
                order.side,
                order.quantity,
                order.open_price,
                newMargin,
    ...
    const errorProfit = useMemo(() => {
        ...
        if (adjustType === ADJUST_TYPE.REMOVE) {
            if (minMarginRatio === null) {
                ...
                return t(`futures:mobile:adjust_margin:max_removable`, {
                    max: formatNumber(
                        maxRemovable,
    ...
    const options = [
        {
            ...
            value: ADJUST_TYPE.ADD,
        },
        {
            title: t("futures:mobile:adjust_margin:remove_margin"),
            value: ADJUST_TYPE.REMOVE,
        },
    ];
        ...
        <>
            <div className="px-4 pt-6">
                <div className="uppercase text-xs text-onus-textSecondary leading-[1.125rem] pb-2">
                    {t('futures:mobile:adjust_margin:adjustment_type')}
                </div>
                <Selectbox
                    options={options}
                    value={adjustType}
                    onChange={(e) => setAdjustType(e)}
                    keyExpr="value"
                    displayExpr="title"
                />
                <div className="uppercase text-xs text-onus-textSecondary leading-[1.125rem] pb-2 mt-4">
                    {t("futures:mobile:adjust_margin:amount")}
                </div>
                <ErrorToolTip message={!errorProfit ? error : ""}>
                    <div className="flex justify-between items-center pl-4 bg-onus-input2 text-sm rounded-md h-11">
                        <NumberFormat
                            thousandSeparator
                            allowNegative={false}
                            ...
                            placeholder={t(
                                "futures:mobile:adjust_margin:amount_placeholder"
                            )}
                        ...
                        >
                            ...
                            <div className="h-full leading-[2.75rem] bg-onus-grey2 w-16 text-onus-grey font-medium rounded-r-md text-center">
                ...
                <div className="mt-4 space-y-2">
                    <div className="text-xs leading-[1.125rem]">
                        <span className="text-onus-textSecondary mr-1">
                            {t("futures:mobile:adjust_margin:assigned_margin")}
                        </span>
                    ...
                    <div className="text-xs leading-[1.125rem]">
                        <span className="text-onus-textSecondary mr-1">
                            {t("futures:mobile:adjust_margin:available")}
                        </span>
                        <span className="text-onus-white font-medium">
                            {formatNumber(
                                floor(+available, assetConfig?.assetDigit),
                                assetConfig?.assetDigit
                            )}
                            <span className="ml-1">
                                {assetConfig?.assetCode}
                    ...
                    <div className="text-xs leading-[1.125rem]">
                        <span className="text-onus-textSecondary mr-1">
                            {t("futures:mobile:adjust_margin:profit_ratio")}
                        </span>
                        <span
                ...
                {errorProfit && (
                    <div className="mt-6 leading-3">
                        <div className="flex items-start">
                            ...
                            <span className="text-xs text-[#FF9F1A]">
                                {errorProfit}
                            </span>
                        </div>
                    </div>
                ...
                <div
                    className={classNames(
                        "flex justify-center items-center bg-onus-base align-middle h-12 text-onus-white rounded-md font-bold mt-6",
                        {
                            "!bg-onus-base opacity-30":
                                !!error || !+amount || !!errorProfit,
                        }
                    )}
                    onClick={() => {
                        if (
                ...
                >
                    {submitting ? (
                        ...
                        <span>
                            {t("futures:mobile:adjust_margin:confirm_btn")}
                        </span>
                    )}
                </div>
            </div>
...
const ErrorToolTip = ({ children, message }) => {
    return (
        <div className="relative">
            <div
                className={classNames(
                    "absolute -top-1 -translate-y-full z-50 flex flex-col items-center",
                    {
                        hidden: !message,
                    }
                )}
