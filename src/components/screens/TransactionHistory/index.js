...
const TransactionHistory = ({ id }) => {
    ...
    useEffect(() => {
        ...
        const fetchTransactionHistory = async () => {
            ...
            //Reset currentPage to 0 when switch tab
            if (previousId !== id) {
                setCurrentPage(0);
                return;
            }
    ...
    const columns = useMemo(() => {
        return {
            ...
            fromAsset: {
                ...
                render: (_row, item) => {
                    ...
                    if (isConvertToNami) {
                        return (
                            <div className="w-full flex justify-end">
                                <div className="w-max border-b border-divider dark:border-divider-dark border-dashed cursor-pointer">
                                    {t('wallet:crypto_list')}
                                </div>
                            </div>
                        );
                    }
                    ...
                    if (value && fromAsset) {
                        return (
                            <div>
                                {formatNumber(value, decimal)} {fromAsset}
                            </div>
                        );
                    } else {
                        return '_';
            ...
            toAsset: {
                ...
                render: (_row, item) => {
                    ...
                    if (value && toAsset) {
                        return (
                            <div>
                                {formatNumber(value, decimal)} {toAsset}
                            </div>
                        );
                    } else {
                        return '_';
            ...
            convert_rate: {
                ...
                render: (_row, item) => {
                    ...
                    if (fromAsset && toAsset) {
                        return (
                            <div>
                                1 {baseAsset} = {formatNumber(rate, decimal)} {displayingPriceAsset}
                            </div>
                        );
                    } else {
                        return '_';
            ...
            fromWallet: {
                ...
                width: 148,
                render: (_row, item) =>
                    item?.kind?.toLowerCase() === 'staking' ? namiSystem[language] : item?.fromUserId ?? renderWalletType(item?.from_wallet)
            },
            toWallet: {
                key: 'toWallet',
                dataIndex: 'to_wallet',
            ...
            amount: {
                ...
                render: (_row, item) => {
                    ...
                    return (
                        <div>
                            {formatPrice(item?.amount || item?.money_use || item?.value, config?.assetDigit ?? 0)} {config?.assetCode ?? 'VNDC'}
                        </div>
                    );
                }
            },
    ...
    }, [t, categoryConfig, assetConfig, id, language]);
