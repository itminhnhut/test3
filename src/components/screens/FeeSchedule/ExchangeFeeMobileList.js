...
export default function ExchangeFeeMobileList({ t }) {
    return (
        <div className="divide-y divide-divider dark:divide-divider-dark space-y-8">
            {FEE_TABLE.map((item) => {
                return (
                    <div key={item.level} className="pt-8">
                        <div className="flex justify-between mb-6">
                            ...
                            <div>
                                <p className="text-xs text-txtSecondary dark:text-txtSecondary-dark mb-1 text-right">NAMI</p>
                                <span className="font-semibold">≥ {formatNumber(item.nami_holding, 0)} NAMI</span>
                            </div>
                        </div>
