...
import { FutureInsurance, PortfolioIcon } from 'components/svg/SvgIcon';
...
const AVAILBLE_KEY = 'insurance_available';
const INSURANCE_ASSET = ['VNDC', 'USDT'];

const InsuranceWallet = ({ estBtc, estUsd, usdRate, marketWatch, isSmallScreen, isHideAsset, setIsHideAsset }) => {
    // Init State
    const [state, set] = useState(INITIAL_STATE);
    const setState = (state) => set((prevState) => ({ ...prevState, ...state }));

    // Rdx
    const wallets = useSelector((state) => state.wallet.INSURANCE);
    ...
    const walletMapper = (allWallet, assetConfig) => {
        ...
        if (Array.isArray(assetConfig) && assetConfig?.length) {
            const futures = assetConfig.filter((o) => INSURANCE_ASSET.includes(o?.assetCode));
            futures &&
                futures.forEach(
                    (item) =>
                        allWallet?.[item.id] &&
    ...
    return (
        <div>
                ...
                <div className="text-base border-b border-divider dark:border-divider-dark pb-5 md:pb-8 flex justify-between items-end">
                    ...
                    <div className="hidden md:block">
                        <div className="flex space-x-3 justify-end h-full w-full">
                            <ButtonV2
                                onClick={() => dispatch(setTransferModal({ isVisible: true, fromWallet: WalletType.INSURANCE, toWallet: WalletType.SPOT }))}
            ...
            <div className="mt-12 md:mt-16 flex items-center justify-between">
                <div className="t-common-v2 hidden md:block">{language === LANGUAGE_TAG.VI ? 'Ví' : ''} Insurance</div>
                {isSmallScreen ? (
...
const renderOperationLink = (assetName, translator, dispatch) => {
    return (
        <ButtonV2
            variants="text"
            onClick={() => dispatch(setTransferModal({ isVisible: true, fromWallet: WalletType.INSURANCE, toWallet: WalletType.SPOT, asset: assetName }))}
...
export default InsuranceWallet;
