...
const FarmingWallet = memo(({ summary, loadingSummary }) => {
        ...
        return (
            <div className="relative z-20 md:max-w-[88%] flex flex-wrap">
                ...
                <div className="mt-4 w-full sm:mt-0 sm:w-1/2 md:w-1/3">
                    ...
                    <div className="mt-1.5 text-sm xl:mt-3 xl:text-[16px] 2xl:text-[18px] text-txtSecondary dark:text-txtSecondary-dark invisible">
                        {loadingSummary ? (
                            <Skeletor width={65} />
                        ) : state.hideAsset ? (
                            SECRET_STRING
    ...
    const renderTableTab = useCallback(() => {
        return (
            <div className="flex items-center px-4 md:px-5">
                <div
                    className="flex flex-col items-center justify-center mr-8 lg:mr-12 cursor-pointer select-none"
                    onClick={() => setState({ tableTab: 0, page: 1 })}
                >
                    <div
                        className={
                            state.tableTab === 0
                                ? 'font-bold text-sm 2xl:text-[16px] pb-2 2xl:pb-4'
                ...
                <div className="flex flex-col items-center justify-center cursor-pointer select-none" onClick={() => setState({ tableTab: 1, page: 1 })}>
                    <div
                        className={
                            state.tableTab === 1
                                ? 'font-bold text-sm 2xl:text-[16px] pb-2 2xl:pb-4'
    ...
    const renderOrder = useCallback(() => {
        ...
        const columns = [
            ...
            {
                key: 'accruing_interest',
                dataIndex: 'accruing_interest',
                title: t('wallet:earn_table.accruing_interest', { action: 'Farm' }),
                align: 'right',
                width: 128
            },
    ...
    const renderPagination = useCallback(() => {
        return (
            <div className="pb-8 pt-4 lg:pt-8 lg:pb-10 2xl:pt-10 2xl:pb-16 flex items-center justify-center">
                <RePagination total={state.tableData?.count} current={state.page} pageSize={PAGE_SIZE} onChange={(page) => setState({ page })} />
            </div>
        );
    }, [state.tableData]);
            ...
            >
                <div className="w-full">
                    ...
                    {!!!Object.keys(state.apiResponse)?.length ? (
                        <div className="text-sm 2xl:text-[16px] text-center font-medium mt-2">{t('wallet:earn_prompt.cancel_message', { action, id })} ?</div>
                    ) : (
                    ...
                    <div className="mt-4 w-full flex flex-row items-center justify-between">
                        <Button title={t('common:close')} type="secondary" componentType="button" style={{ width: '48%' }} onClick={onCloseModal} />
                        <Button
                            title={state.onCancelling ? <PulseLoader color={colors.white} size={3} /> : t('common:confirm')}
                            type="primary"
    ...
    const renderShowMsg = useCallback(() => {
        return (
            <Modal isVisible={state.openModal?.showMsg} onBackdropCb={() => showMsg(false)}>
                <div className="w-full">
                    <div className="text-center text-md font-bold capitalize">{t('modal:notice')}</div>
                    <div className="mt-3 text-center font-medium text-md">{state?.canceledMsg}</div>
                    <div className="mt-4 w-full flex flex-row items-center justify-center">
                        <Button title={t('common:confirm')} type="primary" componentType="button" style={{ width: '48%' }} onClick={() => showMsg(false)} />
                    </div>
                </div>
            </Modal>
        );
    ...
    return (
        <div>
            <div className="flex items-center">
                <div className="t-common-v2">Farming</div>
                <div
            ...
            <div
                style={currentTheme === THEME_MODE.DARK ? { backgroundColor: 'rgba(0, 200, 188, 0.2)' } : { backgroundColor: 'rgba(222, 247, 229, 0.3)' }}
                className="relative mt-8 px-6 py-4 lg:px-8 lg:py-6 rounded-xl"
            >
                {renderDashboard()}
                <img
                    src={getS3Url('/images/screen/wallet/farming_graphics.png')}
                    alt="Nami.Staking"
    ...
    data.forEach((stake) => {
        if (utils?.tab === 0) {
            result.push({
                ...
                amount: (
                    <span className="text-sm 2xl:text-[16px]">
                        <AssetValue value={stake?.amount} /> <AssetName assetId={stake?.currency} />
                    </span>
                ),
                accruing_interest: (
                    <span className="text-sm 2xl:text-[16px]">
                        <AssetValue value={stake?.estimate_interest_by_day} /> <AssetName assetId={stake?.earn_currency} />
                    </span>
                ),
                ...
                operation: (
                    <div
                        className="text-sm 2xl:text-[16px] text-dominant cursor-pointer hover:opacity-80"
            ...
            result.push({
                ...
                interest: (
                    <span className="text-sm 2xl:text-[16px]">
                        {formatWallet(stake?.interest, 2)} <AssetName assetId={stake?.earn_currency} />
                    </span>
                ),
