...
import { countDecimals, eToNumber, formatNumber, formatTime, formatWallet, getS3Url, shortHashAddress } from 'redux/actions/utils';
...
const ExchangeWithdraw = () => {
    ...
    const assetConfig = useMemo(
        () => state.assetConfigs?.find((o) => o.assetCode === state.selectedNetwork?.coin),
    ...
    const withdraw = async (params = {}, cleverProps = {}, isFromOtpModal) => {
        ...
        try {
            ...
            const result = await withdrawHelper(assetId, amount, network, withdrawTo, tag, otp);
                    ...
                    if (result?.data?.status === ApiStatus.SUCCESS) {
                        ...
                        switch (result?.data?.data) {
                            case WITHDRAW_RESULT.MissingOtp:
                                setState({
                                    errors: {
                                        message: t('wallet:withdraw_prompt.input_otp_suggest')
                                    }
                                });
                                break;
                            case WITHDRAW_RESULT.InvalidOtp:
                                setState({
                                    errors: {
                                        message: <Trans>{t('wallet:errors.wrong_otp')}</Trans>
                                    }
                                });
                                break;
                            ...
                            case WITHDRAW_RESULT.WithdrawDisabled:
                                setState({
                                    errors: {
                                        ...
                                        message: t('wallet:errors.withdraw_disabled')
                                    }
                                });
    ...
    const renderTab = useCallback(() => {
        return (
            <div className="mt-5 ml-4 flex items-end">
                ...
                >
                    <a
                        className={classNames('flex flex-col items-center font-medium text-sm lg:text-[16px] text-txtSecondary dark:text-txtSecondary-dark', {
                            '!font-bold !tex-txtPrimary dark:!text-txtPrimary-dark': state.type === TYPE.crypto
                        })}
                    >
                        <div className="pb-2.5">TOKEN</div>
                        <div className={classNames('w-[32px] h-[3px] md:h-[2px] bg-dominant invisible', { '!visible': state.type === TYPE.crypto })} />
                    </a>
                </Link>
            </div>
        );
            ...
            <>
                <div className="flex items-center">
                    ...
                    <span className="ml-2 font-medium text-sm text-txtSecondary dark:text-txtSecondary-dark">
                        {state.selectedAsset?.fullName || state.selectedAsset?.assetCode}
                    </span>
                </div>
    ...
    const renderCryptoList = useCallback(() => {
        ...
        origin.forEach((cfg) => {
            if (!IGNORE_TOKEN.includes(cfg?.assetCode)) {
                items.push(
                    <Link key={`wdl_cryptoList__${cfg?.assetCode}`} href={withdrawLinkBuilder(state.type, cfg?.assetCode)} prefetch={false}>
                        <a
                            className={
    ...
    const renderNetworkInput = useCallback(() => {
        return (
            <>
                <div className={state.selectedNetwork?.withdrawEnable ? 'flex items-center' : 'flex items-center opacity-40'}>
                    <span className="font-bold text-sm text-dominant">{getTokenType(state.selectedNetwork?.network) || '--'}</span>
    ...
    const renderAddressInput = useCallback(() => {
        return (
            ...
            >
                ...
                <span
                    className={
                        state.address ? 'font-bold text-sm text-dominant hover:opacity-80' : 'font-bold text-sm text-dominant hover:opacity-80 invisible'
                    }
    ...
    const renderAmountInput = useCallback(() => {
        return (
            ...
            >
                ...
                />
                <span
                    className={state.amount ? 'font-bold text-sm text-dominant hover:opacity-80' : 'font-bold text-sm text-dominant hover:opacity-80 invisible'}
    ...
    const renderNetworkList = useCallback(() => {
        ...
        state.selectedAsset.networkList.forEach((nw) => {
            items.push(
                ...
                >
                    <div>
                        <span className="text-sm font-medium">{getTokenType(nw?.network)}</span>
    ...
    const renderAssetAvailable = useCallback(() => {
        ...
        return (
            <div className="flex items-center text-sm">
                <span className="text-txtSecondary dark:text-txtSecondary-dark mr-2">{t('common:available_balance')}</span>
                <span className="font-bold flex items-center">
                    {avbl === '0' ? '0.0000' : avbl} <span className="ml-1">{state.selectedAsset?.assetCode}</span>
                </span>
            </div>
        );
    ...
    const renderWdlFee = useCallback(() => {
        return (
            <div className="flex items-center text-sm mt-2.5">
                <span className="text-txtSecondary dark:text-txtSecondary-dark">{t('wallet:withdraw_fee')}</span>
                <span className="font-bold ml-2">
                    {state.withdrawFee?.amount ? formatWallet(state.withdrawFee?.amount, state.feeCurrency?.assetDigit) : state.selectedNetwork?.withdrawFee}{' '}
                    {state.withdrawFee && state.feeCurrency ? state.feeCurrency?.assetCode : state.selectedNetwork?.coin}
                </span>
            </div>
        );
    ...
    const renderMinWdl = useCallback(() => {
        let min;
        if (state.withdrawFee?.amount >= state.selectedNetwork?.withdrawMin && state.selectedAsset?.assetCode === state.feeCurrency?.assetCode) {
            min = `${formatWallet(state.withdrawFee?.amount, assetConfig?.assetDigit)} ${state.selectedAsset?.assetCode}`;
        } else {
            min = `${formatWallet(+state.selectedNetwork?.withdrawMin, assetConfig?.assetDigit)} ${state.selectedAsset?.assetCode} `;
        }

        return (
            <div className="flex items-center text-sm mt-2.5">
                <span className="text-txtSecondary dark:text-txtSecondary-dark">{t('wallet:min_withdraw')}</span>
                <span className="font-bold ml-2">{min || '--'}</span>
            </div>
        );
    }, [state.selectedNetwork, state.selectedAsset, state.withdrawFee, state.feeCurrency, assetConfig]);
    ...
    const renderMaxWdl = useCallback(() => {
        ...
        return (
            <div className="flex items-center text-sm mt-2.5">
                <span className="text-txtSecondary dark:text-txtSecondary-dark">{t('wallet:max_withdraw')}</span>
                <span className="font-bold ml-2">
                    {state.selectedNetwork?.maxWithdraw ? formatWallet(state.selectedNetwork.maxWithdraw?.value, state.selectedNetwork?.assetDigit) : '--'}{' '}
    ...
    const renderInputIssues = useCallback(
        (type) => {
            ...
            if (type === 'address') {
                if (state?.validator?.hasOwnProperty('address') && state.address) {
                                    ...
                                    {t('wallet:errors.invalid_min_amount', {
                                        value:
                                            state.withdrawFee?.amount >= state.selectedNetwork?.withdrawMin
                                                ? formatWallet(state.withdrawFee?.amount, assetConfig?.assetDigit)
                                                : formatWallet(state.selectedNetwork?.withdrawMin, assetConfig?.assetDigit)
                                    })}{' '}
                            ...
                            inner = (
                                <span className="block w-full font-medium text-red text-xs lg:text-sm text-right mt-2">
                                    {t('wallet:errors.decimal_scale_limit', {
                                        value: countDecimals(eToNumber(state.selectedNetwork?.withdrawIntegerMultiple))
                                    })}
                                </span>
                            );
                            break;
            ...
            if (type === 'memo') {
                if (state.memo && state?.validator?.hasOwnProperty('memo')) {
                    if (state?.validator.memo) {
                        ...
                        inner = <span className="block w-full font-medium text-red text-xs lg:text-sm text-right mt-2">{t('wallet:errors.invalid_memo')}</span>;
                    }
                } else {
                    inner = null;
                }
        ...
        [state.validator, state.address, state.selectedNetwork, state.selectedAsset, state.withdrawFee, assetConfig]
    );
        ...
        return (
            <div className="relative mt-5">
                ...
                >
                    ...
                    <span
                        className={
                            state.memo ? 'font-bold text-sm text-dominant hover:opacity-80' : 'font-bold text-sm text-dominant hover:opacity-80 invisible'
                        }
    ...
    const renderWithdrawHistory = useCallback(() => {
        const data = dataHandler(state.histories, state.loadingHistories, paymentConfigs, {
            getAssetName,
            t
        });
        let tableStatus;
    ...
    const renderWdlConfirm = useCallback(() => {
        ...
        const isSuccess = state.withdrawResult?.status === ApiStatus.SUCCESS && !!Object.keys(state.withdrawResult?.data)?.length;
        ...
        return (
            <Modal type="confirmation" isVisible={state.openWithdrawConfirm} title={title} titleStyle="uppercase" className="md:px-6 md:pb-6 relative" noButton>
                <div className="mt-6 w-[280px] md:min-w-[410px]">
                        ...
                        <>
                            <div className="text-sm mb-2 flex items-center justify-between">
                                ...
                                <span className="font-medium">
                                    ...
                                    {state.selectedAsset?.fullName && <span className="ml-1">({state.selectedAsset?.fullName})</span>}
                                </span>
                            </div>
                            <div className="text-sm mb-2 flex items-center justify-between">
                                <span className="text-txtSecondary dark:text-txtSecondary-dark">{t('wallet:receive_address')}</span>
                                <span className="font-medium cursor-pointer" title={`${t('wallet:receive_address')}: ${state.address}`}>
                            ...
                            <div className="text-sm mb-2 flex items-center justify-between">
                                ...
                                <span className="font-medium">
                                    {formatWallet(state.amount, assetConfig?.assetDigit)} {state.selectedAsset?.assetCode}
                                </span>
                            </div>
                            <div className="text-sm mb-2 flex items-center justify-between">
                                <span className="text-txtSecondary dark:text-txtSecondary-dark">{t('common:fee')}</span>
                                <span className="font-medium">
                                    {formatWallet(state.withdrawFee?.amount, assetConfig?.assetDigit)} {state.feeCurrency?.assetCode}
                                </span>
                            </div>
                            <div className="text-sm mb-2 flex items-center justify-between">
                                <span className="text-txtSecondary dark:text-txtSecondary-dark">{t('wallet:will_receive')}</span>
                                <span className="font-medium">
                                    {formatWallet(+state.amount - state.withdrawFee?.amount, assetConfig?.assetDigit)} {state.selectedAsset?.assetCode}
                                </span>
                            </div>
                            <div className="text-sm mb-2 flex items-center justify-between">
                            ...
                            {!state.memo && <div className="mt-2 text-xs text-red">* {t('wallet:notes.memo_wdl_tips')}</div>}
                        </>
                    )}
                    ...
                    {state.withdrawResult && !isErrors && !isSuccess && (
                        <div className="">
                            {state.otpModes.includes('email') && (
                                <div>
                                    <div className="font-medium text-sm ">{t('common:email_authentication')}</div>
                            ...
                            {state.otpModes.includes('tfa') && (
                                <div className="mt-6">
                                    <div className="font-medium text-sm ">{t('common:tfa_authentication')}</div>
                            ...
                            {state.errors && <div className="mt-4 text-red text-xs lg:text-sm font-medium text-center">{state.errors?.message}</div>}
                        </div>
                    )}
                    ...
                    {!isSuccess && state.errors?.status && (
                        <div className="w-full flex flex-col items-center justify-center">
                            <img src={getS3Url('/images/icon/errors.png')} className="w-[65px] h-[65px]" alt={`ERROR: ${state.errors?.status}`} />
                            <div className="mt-5">
                                <div className="text-center uppercase font-medium">
                                    <span className="text-txtSecondary dark:text-txtSecondary-dark">{t('common:error_code')}:</span>
                                    <span className="uppercase">{state.errors?.status}</span>
                                </div>
                    ...
                    {isSuccess && !isErrors && (
                        <div className="w-full flex flex-col items-center justify-center">
                            <img src={getS3Url('/images/icon/success.png')} className="w-[65px] h-[65px]" alt="SUCCESS" />
    ...
    const renderKycRequiredModal = useCallback(() => {
        ...
        return (
            <ModalV2 isVisible={isOpenModalKyc} onBackdropCb={() => setIsOpenModalKyc(false)} className="!max-w-[400px]" wrapClassName="px-6 flex flex-col">
    ...
    useEffect(() => {
        if (paymentConfigs && state.asset) {
            const selectedAsset = find(paymentConfigs, (o) => o?.assetCode === state.asset);
    ...
    useEffect(() => {
        if (state.withdrawFee && paymentConfigs) {
            const cfg = paymentConfigs?.filter((e) => e.assetId === state.withdrawFee?.currency)?.[0];
            cfg &&
                setState({
                    feeCurrency: {
    ...
    useEffect(() => {
        setState({
            validator: withdrawValidator(
                ...
                state.withdrawFee?.amount >= state.selectedNetwork?.withdrawMin ? state.withdrawFee?.amount : state.selectedNetwork?.withdrawMin,
                state.selectedNetwork?.withdrawMax?.value,
                assetBalance?.value - assetBalance?.locked_value,
                state.selectedNetwork?.withdrawEnable
            )
        });
    }, [state.selectedAsset, state.amount, state.address, state.memo, state.selectedNetwork, state.withdrawFee, assetBalance]);
        ...
        <MaldivesLayout>
            <Background isDark={currentTheme === THEME_MODE.DARK}>
                <div className="mal-container px-4">
                    ...
                    <div className="t-common mt-11">{t('wallet:withdraw_history')}</div>
                    <MCard addClass="mt-8 py-0 px-0 overflow-hidden">{renderWithdrawHistory()}</MCard>
                    {renderPagination()}
                </div>
                {renderWdlConfirm()}
...
function dataHandler(data, loading, configList, utils) {
    ...
    data.forEach((h) => {
        ...
        if (url) {
            txhashInner = (
                <a href={url} target="_blank" className="!text-sm whitespace-nowrap cursor-pointer hover:text-dominant hover:!underline">
        ...
        result.push({
            ...
            amount: <span className="!text-sm whitespace-nowrap">{formatNumber(amount, 4, 2)}</span>,
            network: <span className="!text-sm whitespace-nowrap">{network}</span>,
            withdraw_to: <span className="!text-sm whitespace-nowrap">{shortHashAddress(address, 5, 5)}</span>,
            txhash: txhashInner,
