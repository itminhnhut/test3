...
import SideOrder from 'components/screens/Nao_futures/Futures/SideOrder';
import OrderLeverage from 'components/screens/Nao_futures/Futures/PlaceOrder/OrderLeverage';
// import ExpiredModal from 'components/screens/Nao_futures/ExpiredModal'
...
const PlaceOrder = ({ decimals, side, setSide, pair, isAuth, availableAsset, pairConfig, isVndcFutures, collapse, onBlurInput, decimalSymbol }) => {
    ...
    const onChangeQuoteQty = (price, leverage) => {
        ...
        let _quoteQty = +Number(maxQuoteQty * (initPercent / 100)).toFixed(decimalSymbol);
    ...
    const inputValidator = (mode) => {
        ...
        switch (mode) {
            case 'quoteQty':
                ...
                const _displayingMax = `${formatNumber(_max, decimalSymbol, 0, true)} ${pairConfig?.quoteAsset}`;
                const _displayingMin = `${formatNumber(_min, decimalSymbol, 0, true)} ${pairConfig?.quoteAsset}`;
                if (quoteQty < +_min) {
                    msg = `${t('futures:minimum_qty')} ${_displayingMin} `;
                    isValid = false;
                } else if (quoteQty > +Number(_max).toFixed(decimalSymbol)) {
            ...
            case 'take_profit':
                ...
                if ((mode === 'stop_loss' && !sl) || (mode === 'take_profit' && !tp)) {
                    return {
                        isValid,
                        msg
                    };
    ...
    const onConfirmEditVolume = (quoteQty) => {
        const _price = type === FuturesOrderTypes.Market ? (VndcFutureOrderType.Side.BUY === side ? _pairPrice?.ask : _pairPrice?.bid) : price;
        ...
        <>
            <div className="flex flex-wrap justify-between px-4 pt-6 pb-[10px] relative z-10 bg-bgPrimary dark:bg-bgPrimary-dark">
                ...
                <div className={collapse ? 'hidden' : 'w-full flex flex-wrap justify-between'}>
                    ...
                    <OrderInput>
                        <OrderMarginMobile marginAndValue={marginAndValue} pairConfig={pairConfig} availableAsset={availableAsset} decimal={decimalSymbol} />
                    </OrderInput>
