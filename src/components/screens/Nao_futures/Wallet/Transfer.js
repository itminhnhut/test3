...
import SelectWalletType from 'components/screens/Nao_futures/Wallet/SelectWalletType';
import SliderAmount from 'components/screens/Nao_futures/Wallet/SliderAmount';
...
export default function Transfer() {
    ...
    const {
        ...
        const allWallets = fromWallet === WalletType.FUTURES ? allFuturesWallet : allExchangeWallet;
    ...
    const { walletFromOptions, walletToOptions } = useMemo(() => {
        const { fromWallets = [], toWallets = [] } = getWalletsAllowedAsset(asset);
        return {
        ...
        switch (data.status) {
            ...
            case TransferWalletResult.INVALID_CURRENCY: {
                message = t('error:ASSET_NOT_SUPPORT', {
                    ['function']: language === LANGUAGE_TAG.VI ? `để Chuyển Ví` : `for Transfer`
                });
                break;
            }
        }
            ...
            <div className="h-[calc(100vh-70px)]">
                <div className="market-mobile px-4 py-6">
                    <div className="flex items-center">
                        <span className="mr-2 text-xl font-semibold">{t('wallet:transfer_asset', { asset })}</span>
                        <AssetLogo size={30} assetCode={asset} />
                    </div>
                    <div className="grid grid-cols-2 gap-4 my-4">
                        <div>
                            <div className="mb-2 text-sm font-medium text-txtSecondary dark:text-txtSecondary-dark">{t('common:from')}</div>
                            <SelectWalletType
                        ...
                        <div className="">
                            <div className="mb-2 text-sm font-medium text-txtSecondary dark:text-txtSecondary-dark">{t('common:to')}</div>
                            <SelectWalletType
                    ...
                    <div>
                        <div className="flex justify-between items-start mb-2">
                            <span className="text-txtSecondary dark:text-txtSecondary-dark text-sm font-medium">{t('common:ext_gate:amount')}</span>
                            {error && <span className="text-yellow text-xs font-medium italic text-right">{error}</span>}
                        </div>
                        <div className="flex bg-gray-5 dark:bg-darkBlue-3 rounded py-2 px-3">
                            ...
                            <div className="flex items-center text-xs font-medium">
                                <span className="text-teal" onClick={handleFillMaxAmount}>
                                    {t('common:max')}
                                </span>
                    ...
                    <div className="text-sm space-y-1">
                        <div className="flex justify-between">
                            <span className="font-medium text-txtSecondary dark:text-txtSecondary-dark">{t('common:ext_gate:available')}</span>
                            <span className="font-semibold">
                                {formatAvl(currentWallet.available, DECIMAL_SCALES[assetConfig.id])}
                                &nbsp;{asset}
                            </span>
                        </div>
                        <div className="flex justify-between whitespace-nowrap">
                            <span className="font-medium text-txtSecondary dark:text-txtSecondary-dark">{t('wallet:mobile:fee')}</span>
                            <span className="font-semibold whitespace-nowrap">
                                {formatNumber(fee, DECIMAL_SCALES[assetConfig.id])}
                                &nbsp;{asset}
                            </span>
                        </div>
                    </div>
                    <div className="mt-auto grid grid-cols-2 gap-2">
                        ...
                        <div
                            className={classNames('h-11 bg-teal rounded-md flex items-center justify-center font-semibold text-sm text-white', {
                                '!pointer-events-none !bg-gray-2 !dark:bg-darkBlue-3': !fromWallet || !toWallet || !amount || error
                            })}
                            onClick={handleTransfer}
                        >
                            {isPlacingOrder ? <PulseLoader color={colors.white} size={3} /> : t('wallet:mobile:btn_confirm_transfer')}
                        </div>
                    </div>
                </div>
            </div>
            <Modal isVisible={visibleSuccessModel} containerClassName="px-6 py-8 !min-w-[18rem] !top-[50%]" noButton>
                <div className="absolute right-0 top-0 p-3">
                    <X size={16} className="cursor-pointer hover:text-dominant" onClick={() => setVisibleSuccessModel(false)} />
                </div>
                <img className="mx-auto" src={getS3Url('/images/screen/wallet/coins_pana.png')} width={150} height={150} />
                <p className="text-center font-semibold text-lg mt-5">{t('wallet:mobile:transfer_asset_success', { asset })}</p>
                <div className="mt-7 mb-8 space-y-4">
                    ...
                    <div className="flex justify-between text-xs">
                        ...
                        <span className="font-semibold">
                            {formatNumber(amount, DECIMAL_SCALES[assetConfig.id])}
                            &nbsp;{asset}
                        </span>
                    </div>
                    <div className="flex justify-between text-xs">
                        <span className="font-medium text-txtSecondary dark:text-txtSecondary-dark whitespace-nowrap mr-3">
                ...
                <Button componentType="button" title={t('common:cancel')} onClick={() => setVisibleSuccessModel(false)} />
            </Modal>
        </>
    );
}
