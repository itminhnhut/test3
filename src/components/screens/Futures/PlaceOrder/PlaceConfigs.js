...
const PlaceConfigs = ({ pairConfig, userSettings, leverage, setLeverage, isVndcFutures, isAuth }) => {
    ...
    return (
        <>
            <div className="mt-5 mb-3 flex items-center w-full">
                <div className="flex-grow flex items-center w-full">
                    <div
                        ...
                        className={`${
                            isVndcFutures ? 'cursor-auto text-gray dark:text-darkBlue-4 ' : ''
                        } px-[16px] py-1 mr-2.5 text-xs font-bold bg-bgSecondary dark:bg-darkBlue-3 cursor-pointer hover:opacity-80 rounded-md`}
                    >
            ...
            {isAuth && !isVndcFutures && (
                <FuturesPreferences isVisible={!!isActive?.preferences} positionMode={positionMode} onClose={() => closePopup('preferences')} />
            )}
