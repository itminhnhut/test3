...
export const getMaxQuoteQty = (price, type, side, leverage, availableAsset, pairPrice, pairConfig, isQuoteQty, isAuth) => {
    ...
    const lotSize =
        pairConfig?.filters?.find((o) =>
            [FuturesOrderTypes.Market, FuturesOrderTypes.StopMarket].includes(_type) ? o?.filterType === 'MARKET_LOT_SIZE' : o?.filterType === 'LOT_SIZE'
        ) || {};
...
export const validator = (key, price, type, side, lastPrice, pairConfig, decimal, t, isText = false) => {
    let isValid = true;
    let msg = null;
    let min = 0,
        max = 0;
