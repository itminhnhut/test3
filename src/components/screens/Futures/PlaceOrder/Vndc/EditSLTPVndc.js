...
const FuturesEditSLTPVndc = ({ isVisible, order, onClose, status, onConfirm, pairTicker, lastPrice = 0, isMobile, onusMode = false }) => {
    ...
    const [data, setData] = useState({
        ...
        price: +(status === VndcFutureOrderType.Status.PENDING
            ? order?.price
    ...
    const onHandleChange = (key, e) => {
        ...
        if (tab === 1) {
            ...
            const { quantity, open_price, status, price, leverage } = order;
            const openPrice = status === VndcFutureOrderType.Status.PENDING ? price : open_price;
    ...
    const inputValidator = (type, price) => {
        ...
        switch (type) {
            ...
            case 'take_profit':
                ...
                if ((type === 'stop_loss' && !data.sl) || (type === 'take_profit' && !data.tp)) {
                    return {
                        isValid,
                        msg
                    };
    ...
    return (
        <ModalV2 isVisible={isVisible} onBackdropCb={onClose}>
            <div
                className={`px-5 py-4 flex items-center justify-between border-b border-divider sticky top-0 z-[10] bg-white rounded-t-lg ${
                    isMobile ? 'dark:bg-[#243042] dark:border-onus-line' : 'dark:bg-darkBlue-2 dark:border-divider-dark'
                }`}
            >
                <span className="font-bold text-[16px]">{t('futures:tp_sl:modify_tpsl')}</span>{' '}
                <X size={20} strokeWidth={1} className="cursor-pointer" onClick={onClose} />
            </div>
            <div className="px-5 pt-4 pb-6 text-sm">
                ...
                <div className="mb-3 font-medium flex items-center justify-between">
                    {status === VndcFutureOrderType.Status.PENDING && order?.type !== VndcFutureOrderType.Type.MARKET && order?.type ? (
                        <div
                            className={`px-3 flex items-center w-full h-[36px] bg-gray-5 ${
                                isMobile ? 'dark:bg-onus-bg2' : 'dark:bg-darkBlue-3'
                            } rounded-[4px] justify-between`}
                        >
                            <TradingInput
                                onusMode={onusMode}
                                thousandSeparator
                ...
                {isMobile ? (
                    <div className="relative">
                        <div className="relative z-[10] px-[20px] text-gray-1 flex items-center justify-around mx-[-20px] w-[calc(100% + 40px)] mt-5 font-semibold">
                            <TabItem isMobile active={tab === 1} onClick={() => setTab(1)} isDark={currentTheme === THEME_MODE.DARK}>
                                {t('futures:price')}
                            </TabItem>
                            <TabItem isMobile active={tab === 0} onClick={() => setTab(0)} isDark={currentTheme === THEME_MODE.DARK}>
                                {t('futures:order_table:profit')}
                            </TabItem>
                            <TabItem isMobile active={tab === 2} onClick={() => setTab(2)} isDark={currentTheme === THEME_MODE.DARK}>
                    ...
                    <div className="px-[20px] text-gray-1 flex items-center justify-around mx-[-20px] bg-[#F2FCFC] dark:bg-darkBlue-4 w-[calc(100% + 40px)] mt-5 font-semibold">
                        <TabItem active={tab === 1} onClick={() => setTab(1)} isDark={currentTheme === THEME_MODE.DARK}>
                            {t('futures:price')}
                        </TabItem>
                        <TabItem active={tab === 0} onClick={() => setTab(0)} isDark={currentTheme === THEME_MODE.DARK}>
                            {t('futures:order_table:profit')}
                        </TabItem>
                        <TabItem active={tab === 2} onClick={() => setTab(2)} isDark={currentTheme === THEME_MODE.DARK}>
                            {t('futures:profit_margin')}
                        </TabItem>
                    </div>
                )}
                <div className="mt-6 flex items-center ">
                    <div className={`px-3 flex items-center w-full h-[36px] bg-gray-5 ${isMobile ? 'dark:bg-onus-bg2' : 'dark:bg-darkBlue-3'} rounded-[4px]`}>
                        <TradingInput
                            onusMode={onusMode}
                            thousandSeparator
                            type="text"
