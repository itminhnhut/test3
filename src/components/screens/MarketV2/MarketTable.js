    ...
    const renderTable = useCallback(() => {
        ...
        if (isMobile) {
            return dataSource.map((e, index) => {
                ...
                return (
                    <div className={classNames('w-full flex justify-between font-normal text-xs', {
                        '': index !== 0,
                        '': index === 0
                    })}>
                        <div className="w-full flex flex-col justify-center items-start">
                            {index === 0 ?
                                <div className="mb-4">{translater('pair')} / {translater('volume_24h')} </div> : null}
                        ...
                        <div className="w-full flex flex-col justify-center items-end">
                            ...
                            <div className="flex h-[64px] items-center">
                                <div
                                    className={classNames('h-9 border flex items-center justify-center rounded-[3px] px-2', {
                ...
                <div
                    ...
                    <div className="w-[calc(100vw-196px)] sm:w-[368px]">
                        <InputV2
                            ...
                            suffix={restProps?.search?.length > 0 &&
                                <X color={isDark ? colors.gray[1] : colors.darkBlue5} size={16}
...
const FavActionButton = ({
    ...
    const callback = _.debounce(async (method, list) => {
        ...
        try {
            ...
            showNotification(
                {
                    message: `FAV_ACTION_UNKNOWN_ERR`,
                    title,
                    type: 'failure'
                },
                2500,
    ...
    return (
        <div className="flex justify-end items-center font-medium text-base">
            ...
            {swapurl ? <Link href={swapurl} prefetch={false}>
                <a className="text-teal re_table__link px-3 flex items-center justify-center border-l-[1px] border-divider-dark"
                   target="_blank">
...
const TokenTypes = ({
    ...
    return <div className="flex items-center space-x-3 h-9 sm:h-12 font-normal text-sm overflow-auto no-scrollbar">
        {types.map(e =>
            <div key={e.id}
                 className={classNames('flex items-center h-full px-4 text-sm sm:text-base font-normal rounded-[800px] border-[1px] border-divider-dark cursor-pointer whitespace-nowrap', {
