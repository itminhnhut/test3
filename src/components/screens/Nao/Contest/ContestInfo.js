...
import { API_CONTEST_GET_USER_DETAIL, API_GET_REFERENCE_CURRENCY } from 'redux/actions/apis';
const ContestInfo = () => {
            ...
            <div className="flex flex-col lg:flex-row flex-wrap gap-5 mt-8 ">
                <CardNao className="!min-h-[136px] !px-6 !py-9 lg:!max-w-[375px]">
                    ...
                    <div className=" text-nao-grey2 text-sm font-medium mt-4 flex sm:flex-col items-center sm:items-start">
                        ...
                        <div className="flex text-nao-grey2 leading-6 ">{t('nao:contest:team_label')}:&nbsp;<span className="text-nao-green font-medium">{userData?.group_name}</span></div>
                    </div>
                </CardNao>
                <CardNao className="!min-h-[136px] !py-7 !px-[35px] w-full lg:w-max">
                    <div className="flex items-center justify-between md:space-x-[70px] flex-wrap md:flex-nowrap">
                        <div className="flex items-center justify-between w-full md:w-1/2">
                            ...
                            <div className="font-semibold leading-8 text-right">{formatNumber(userData?.total_order)}</div>
                        </div>
                        ...
                        <div className="flex items-center justify-between w-full md:w-1/2">
                            ...
                            <div className={`font-semibold leading-8 text-right ${userData?.total_pnl < 0 ? 'text-nao-red' : 'text-nao-green2'}`}>{formatNumber(userData?.total_pnl, 2, 0, true)} VNDC</div>
                        </div>
                    </div>
                    <div className="h-[1px] bg-nao-grey/[0.2] w-full my-2 md:hidden"></div>
                    <div className="flex items-center justify-between md:space-x-[70px] flex-wrap md:flex-nowrap">
                        <div className="flex items-center justify-between w-full md:w-1/2">
                            ...
                            <div className="font-semibold leading-8 text-right">{formatNumber(userData?.total_volume, 2, 0, true)} VNDC</div>
                        </div>
                        <div className="h-[1px] bg-nao-grey/[0.2] w-full my-2 md:hidden"></div>
                        <div className="flex items-center justify-between w-full md:w-1/2">
                            <label className="text-nao-grey2 text-sm leading-6 ">{t('nao:contest:per_pnl')}</label>
                            <div className={`font-semibold leading-8 text-right ${userData?.pnl < 0 ? 'text-nao-red' : 'text-nao-green2'}`}>{formatNumber(userData?.pnl, 2, 0, true)}%</div>
                        </div>
                    </div>
                    <div className="h-[1px] bg-nao-grey/[0.2] w-full my-2 md:hidden"></div>
                    <div className="flex items-center justify-between md:space-x-[70px] flex-wrap md:flex-nowrap">
                        <div className="flex items-center justify-between w-full md:w-1/2">
                            ...
                            <div className="font-semibold leading-8 text-right">#{userData?.individual_rank_volume}</div>
                        </div>
                        <div className="h-[1px] bg-nao-grey/[0.2] w-full my-2 md:hidden"></div>
                        <div className="flex items-center justify-between w-full md:w-1/2">
