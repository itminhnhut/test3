        ...
        <section id="nao_token" className="pt-10 sm:pt-20">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <span className="text-sm sm:text-[1rem] text-txtSecondary dark:text-txtSecondary-dark">{t('nao:nao_token:description')}</span>
                </div>
            </div>
            <div className="pt-6 flex items-center flex-wrap gap-[21px]">
                <CardNao className={`!flex-row items-center !justify-start ${width > 1080 ? '!flex-none' : ''}`}>
                    ...
                    <div className="flex flex-col pl-6">
                        <label className="text-txtPrimary dark:text-txtPrimary-dark text-sm font-medium leading-6">{t('nao:nao_token:nao_token')}</label>
                        <div className="pt-3">
                            <div className="text-gray-15 dark:text-gray-7 font-semibold text-[1.375rem] sm:text-2xl leading-8">1,000 VNDC</div>
                            <span className="text-sm text-txtSecondary dark:text-txtSecondary-dark leading-6">$0.025</span>
                        </div>
                    </div>
                </CardNao>
                <CardNao noBg className="!flex-row items-center flex-wrap w-full gap-4 sm:!justify-between">
                    <div className="flex items-center gap-5 max-w-[300px]">
                        ...
                        <div className="text-lg sm:text-[1.25rem] text-txtPrimary dark:text-txtPrimary-dark font-semibold">{t('nao:nao_token:get_buy_now')}</div>
                    </div>
