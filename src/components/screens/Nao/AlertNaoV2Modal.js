...
const AlertNaoV2Modal = memo(
    forwardRef((props, ref) => {
        ...
        const getIcon = (type) => {
            if (options.current?.icon) {
                return <img src={options.current?.icon} alt="" className="rounded-full object-cover w-20 h-20" />;
            }
            switch (type) {
                case 'success':
                    return <CheckCircle size={80} />;
                case 'error':
                    return <CrossCircle size={80} />;
                case 'warning':
                    return <WarningTriangle size={80} />;
                case 'team':
                    return <TeamIcon />;
                default:
                    return <CheckCircle size={80} />;
        ...
        return (
            <ModalV2 isMobile={isMobile} isVisible={true} onBackdropCb={onCancel} className="!max-w-[488px]">
                <div className="flex flex-col items-center justify-between h-[calc(100%-1.5rem)]">
                    <div className="mb-6">{getIcon(options.current.type)}</div>
                    <div className="text-xl sm:text-2xl font-semibold mb-6 text-center">{options.current.title}</div>
                    {options.current.messages && (
                        <div
                            className={classNames('text-center text-sm sm:text-base text-txtSecondary dark:text-txtSecondary-dark', {
                                'mb-10': !!actions.current?.onConfirm
                            })}
                    ...
                    <div className="flex items-center w-full space-x-4">
                        {actions.current?.onConfirm && (
                            <ButtonNao
                                variant={options.current?.variant ?? 'PRIMARY'}
                                onClick={onConfirm}
                                className={classNames('w-full !rounded-md', options.current?.className)}
                            >
