...
const LuckyPage = ({ ticket, onOpen, width }) => {
        ...
        <>
            <div className="relative z-[2]">
                <div className={classnames(
                    ...
                    {t(`nao:luckydraw:${ticket ? '' : 'not_'}ticket_title`)}
                </div>
                <div className={classnames(
                    'font-medium ',
                    ...
                    {t(`nao:luckydraw:ticket_des`, { vndc: ticket ? '100,000,000' : '10,000,000 ', nao: ticket ? '100' : '10' })}
                </div>
                {ticket && <div onClick={onOpen} className="bg-nao-blue2 font-semibold leading-6 py-3 rounded-xl cursor-pointer">
                    {ticket ? t('nao:luckydraw:open_now') : t('nao:luckydraw:trade')}
                </div>
                }
                {ticket && <div className="text-[0.625rem] opacity-[0.65] leading-7 pt-2">
            ...
            <BgCenter className={`select-none top-1/2  ${xs ? 'min-w-[177px]' : 'min-w-[253px]'}`}>
                ...
                {ticket &&
                    <BgCenter className={`${xs ? 'min-w-[127px]' : 'min-w-[181px]'} top-[25%]`}>
                        <DragTicket onOpen={onOpen} xs={xs} />
                    </BgCenter>
                }
