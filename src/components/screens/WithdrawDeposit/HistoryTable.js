...
const mappingStatus = [...TABS].slice(1, -1).reduce((obj, { key, type, localized }) => ({ ...obj, [key]: { localized, type } }), {});
...
const HistoryTable = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            width: 182,
            render: (row) => {
                const statusContent = mappingStatus?.[row] || { localized: '--', type: TYPES.DEFAULT };
                // const config = assetConfig?.find((e) => e?.id === item?.currency);
                return (
