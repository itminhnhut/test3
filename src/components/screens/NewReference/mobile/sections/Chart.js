...
const Chart = ({ user }) => {
    ...
    return (
        <div className="px-4">
            ...
            <CollapsibleRefCard title={t('reference:tabs.statistic')} isBlack>
                {user ? (
                    <div className="w-auto">
                        ...
                        <div className="mt-6">
                            ...
                            <div className="h-100 mt-4">{renderChart()}</div>
...
const CustomFilter = ({ isShow, onClose, t, filter, onConfirm }) => {
    ...
    return (
        <PopupModal isVisible={isShow} onBackdropCb={onClose} title={t('reference:referral.custom')}>
            ...
            <div
                className="w-full flex bg-teal mt-8 rounded-md h-11 justify-center items-center text-sm font-bold leading-6 text-white"
                onClick={() => _onConfirm()}
            >
                {t('common:confirm')}
            </div>
