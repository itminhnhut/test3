...
const AddNewRef = ({ isShow = false, onClose, doRefresh, defaultRef }) => {
    ...
    const handleAddNewRef = _.throttle(async () => {
        ...
        if (status === 'ok') {
            setResultData({
                message: t('reference:referral.addref_success', { value: refCode }),
                isSucess: true
            })
            // doClose()
        } else {
                    ...
                    <div>
                        ...
                        <div className={classNames('mt-1 rounded-[4px] px-3 h-11 flex justify-between items-center bg-gray-4 font-medium text-sm leading-6 gap-4', { 'border-red border-[1px]': error.length })}>
                            <div className='flex w-full justify-between items-center '>
                                <input className='text-darkBlue font-medium w-full' maxLength={8} placeholder={t('reference:referral.ref_placeholder')} onChange={handleInputRefCode} value={refCode}
