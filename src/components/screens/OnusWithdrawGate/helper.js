...
export const getAvailableToken = (currency, balance) => {
    return balance && balance[currency] ? balance[currency].value - balance[currency].locked_value : 0;
};
...
export function buildNamiExchangeAppLink(rootLink = process.env.NEXT_PUBLIC_APP_LINK_HREF) {
    const referralCode = sessionStorage && sessionStorage.getItem('refCode') ? sessionStorage.getItem('refCode') : '';
