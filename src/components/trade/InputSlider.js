...
const Slider = ({
    ...
    const renderDotAndLabel = () => {
        ...
        for (let i = 0; i <= _dots; ++i) {
            ...
            dot.push(
                <Dot
                    bgColorActive={_bgColorActive}
                    key={`inputSlider_dot_${i}`}
                    active={pos.left >= i * size}
                    percentage={i * size}
                    isDark={isDark}
                    onusMode={onusMode}
                    bgColorDot={_bgColorDot}
                />
            );
            label.push(
    ...
    return (
        <div className="px-2">
            ...
            <Track {...props} onusMode={onusMode} ref={container} onTouchStart={handleTrackMouseDown} onMouseDown={handleTrackMouseDown}>
                <Active onusMode={onusMode} height={height} style={valueStyle} bgColorSlide={_bgColorActive} />
                <SliderBackground onusMode={onusMode} height={height} isDark={isDark} BgColorLine={BgColorLine} />
                <DotContainer>
                    {!customDotAndLabel && <Dot active={pos.left >= 0} percentage={0} isDark={isDark} bgColorActive={bgColorActive} onusMode={onusMode} />}
                ...
                >
                    ...
                    >
                        ...
                        {customPercentLabel
                            ...
                            : showPercentLabel && (
                                  <ThumbLabel isZero={pos.left === 0} min={pos.left === 0} max={pos.left === xmax} isDark={isDark} onusMode={onusMode}>
                                      {ceil(pos.left, 0)}%
                                  </ThumbLabel>
                              )}
                    </Thumb>
