...
import { BxsBellIcon, FutureInsurance, LendingIcon } from '../svg/SvgIcon';

const NOTI_READ = NotificationStatus.DELETED;

const MarkReadAllIcon = () => (
...
const NotificationList = ({ btnClass }) => {
                        ...
                        <div
                            ...
                            onClick={() => {
                                ...
                                if (notification?.context?.isAutoSuggest)
                                    router.push({
                                        pathname: PATHS.PARTNER_WITHDRAW_DEPOSIT.OPEN_ORDER,
                                        query: { suggest: notification?.context?.displayingId }
                                    });
                                closeDropdownPopover();
                            }}
                        >
    ...
    return (
        <>
            {showNotiSetting ? (
                <NotiSettingModal t={t} isMobile={false} isVisible={showNotiSetting} onClose={() => setShowNotiSetting(false)} language={language} />
            ) : null}
                    ...
                    <div className="py-6">
                        <div className="flex items-center px-6 justify-between mb-8">
                            ...
                            <div className="flex gap-4 items-center">
                                ...
                                <div onClick={() => {}} className={classNames('text-sm font-semibold cursor-pointer hover:opacity-70 transition-opacity')}>
                                    <svg
    ...
    return (
        ...
        >
            <div className="text-base font-normal text-txtPrimary dark:text-txtPrimary-dark">
                <div>
                    <div className="text-2xl font-semibold mb-6">{t('navbar:noti_setting')}</div>
                    <div className="pb-6 w-full border-b-[1px] border-b-divider dark:border-b-divider-dark">
                        <NotiToggle
                            updating={updating}
                            text={t(`navbar:noti_content.${NOTI_GROUP_KEYS.ALL.toLocaleLowerCase()}`, {
                                defaultValue: notiGroup?.[NOTI_GROUP_KEYS.ALL]?.[language]
                            })}
                            loading={loading}
                            isAvailable
