...
import {
    ComponentSortItem,
    ComponentSortWrapper, ComponentTabItem, ComponentTabWrapper,
    ComponentTitle, DateRange, ReferralNote, TimeRange
} from "../styledReference";
import {DashboardGraphics, DashboardItem, DashboardWrapper} from "./styledDashboard";
import {formatNumber, formatTime, getTokenIcon} from "../../../utils/reference-utils";
...
const DASHBOARD_SVG = <svg width="45" height="45" viewBox="0 0 45 45" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <path d="M3.51562 30.9375H7.73438V23.9062H3.51562V30.9375ZM4.92188 25.3125H6.32812V29.5312H4.92188V25.3125Z" fill="black"/>
    <path d="M9.14062 30.9375H13.3594V25.3125H9.14062V30.9375ZM10.5469 26.7188H11.9531V29.5312H10.5469V26.7188Z" fill="black"/>
    <path d="M14.7656 30.9375H18.9844V19.6875H14.7656V30.9375ZM16.1719 21.0938H17.5781V29.5312H16.1719V21.0938Z" fill="black"/>
</svg>
const EARNED = {nami: 6868, vndc: 200, usdt: 101.98}

const ReferralDashboard = memo((props) => {
    // @ts-ignore
    const {width, timeSort, setTimeSort, typeSort, setTypeSort, data, user} = props;
    ...
    const renderEarnedCommissionToken = useCallback(() => {
        ...
        for (const [key, value] of Object.entries(commission)) {
            ...
            inner.push(
                <DashboardItem>
                    <div><Translate id='referral_pages.dashboard.you_earned'/> {getTokenIcon(+key, 14)}</div>
                    <div>
    ...
    const renderFriends = useCallback(() => {
        ...
        return (
            <>
                <DashboardItem>
                    <div><Translate id='referral_pages.dashboard.total_of_friends'/></div>
                    <div>{total_friends ? formatNumber(total_friends, 0) : 0}</div>
                </DashboardItem>
                <DashboardItem>
                    <div><Translate id='referral_pages.dashboard.total_of_traded_friends'/></div>
    ...
    const renderTypeSelect = useCallback(() => {
        ...
        return (
            <ComponentTabWrapper>
                ...
                {/*    <Translate id='referral_pages.time_sort.all'/>*/}
                {/*</ComponentTabItem>*/}
                <ComponentTabItem active={dashboard === 2} onClick={() => setTypeSort({...typeSort, dashboard: 2})}>
                    Exchange
                </ComponentTabItem>
                <ComponentTabItem active={dashboard === 3} onClick={() => setTypeSort({...typeSort, dashboard: 3})}>
                    Futures
                </ComponentTabItem>
            </ComponentTabWrapper>
        )
    ...
    const renderTime = useCallback(() => {
        ...
        return (
            <DateRange>
                {updated_at ?
                    <>
                        <Translate id='referral_pages.time_sort.date_range'/>: {width < 768 ? <br/> : <span className='pl-1'/>}
                        <span><Translate id='referral_pages.time_sort.until'/> {formatTime(updated_at, 'HH:mm DD/MM/YYYY')}</span>
                    </>
    ...
    return (
        <>
            <ComponentTitle>
                {DASHBOARD_SVG}
                <div><Translate id='referral_pages.categories.dashboard'/></div>
            </ComponentTitle>
            {renderTypeSelect()}
            <TimeRange>
                {renderTimeSelect()}
            ...
            <DashboardWrapper>
                ...
                <DashboardGraphics>
                    <img src='/images/pages/referral/dashboard_graphics.png' alt=''/>
                </DashboardGraphics>
            </DashboardWrapper>
            <ReferralNote>
                * <Translate id='referral_pages.addition_info.dashboard'/>
            </ReferralNote>
        </>
    )
})
