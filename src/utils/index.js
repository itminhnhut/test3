...
export function isNumeric(val) {
    return (typeof val === 'string' && !!val && !isNaN(+val)) || typeof val === 'number';
}
...
export const getSupportCategories = async (language = 'vi') => {
    ...
    if (allTags) {
        const announcementCategories = allTags
            ?.map((o) => ({
                id: o?.id,
                name: o?.name,
...
export const getLastedArticles = async (tag = '', limit = 10, page = 1, language = 'vi', isHighlighted = false) => {
    ...
    const excludedTags = [
        ...
        'ins-en-news',
        'ins_faq',
        'ins_faq-en-insurance-wallet',
        'ins_faq-en-insurance-margin',
        'ins_faq-en-insurance-history',
        'ins_faq-en-insurance-commission',
        'ins_faq-vi-insurance-wallet',
        'ins_faq-vi-insurance-margin',
        'ins_faq-vi-insurance-history',
        'ins_faq-vi-insurance-commission'
    ];
...
export const substring = (str, start = 10, end = -4) => (String(str).length > 10 ? `${String(str).substr(0, start)}...${String(str).substr(end)}` : str);
