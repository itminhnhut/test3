...
const RewardCenter = () => {
                    ...
                    if (language === LANGUAGE_TAG.VI) {
                        msg = (
                            <>
                                ...
                                <br />
                                Bạn đã nhận được{' '}
                                <span className="ml-0.5 text-dominant">
                                    {formatNumber(data?.data?.reward?.value || reward?.value, assetConfig?.assetDigit || 0)}{' '}
                                    {data?.data?.reward?.asset || assetConfig?.assetCode}
                                </span>
                            </>
                        );
                    } else {
        ...
        if (!data?.length) {
            return (
                <div className="min-h-[400px] xl:min-h-[520px] 2xl:min-h-[550px] h-full flex items-center justify-center dark:bg-darkBlue-2">
                    <Empty message={t('reward-center:no_promo')} messageStyle="text-xs sm:text-sm" />
                </div>
            );
        }
            ...
            >
                ...
                {state.popupMsg?.type === 'success' && (
                    <>
                        ...
                        <div className="w-full">
                            <img
                                className="m-auto w-[220px] h-[220px] md:w-[265px] md:h-[265px]"
                                src={getS3Url('/images/screen/reward/treasure.png')}
                                alt="SUCCESS"
                            />
                        </div>
                    </>
                ...
                <div
                    className={
                        state.popupMsg?.type
                            ? 'mb-4 px-2 sm:px-4 md:px-8 lg:mb-5 leading-6 font-medium text-sm xl:text-[16px] text-center'
    ...
    const renderLunarPopup = useCallback(() => {
        ...
        return (
            <PortalPopup noButton isVisible={!!state.lunarNewYear} onBackdropCb={() => setState({ lunarNewYear: null })} containerClassName="!bg-transparent">
                <div className="relative min-w-[280px] sm:w-[380px] lg:w-[420px] max-w-[420px]">
                    <img className="relative z-10 rounded-[20px]" src={getS3Url(imgSrc)} alt={null} />
    ...
    return (
        <>
            ...
            <div className="h-full">
                ...
                <div className="mt-8 t-common select-none">{isApp ? t('reward-center:title') : t('reward-center:promotion')}</div>
                <div style={rewardAppStyles?.styles} className={rewardAppStyles?.className}>
                    {/*{isApp && <div ref={observe}>{renderSegmentTabs()}</div>}*/}
                    <div id="reward-center" className="mt-6 overflow-hidden rounded-lg dark:border dark:border-divider-dark" style={rewardListWrapperStyles}>
                        {renderReward()}
                    </div>
                </div>
            </div>
...
export const getStaticProps = async ({ locale }) => ({
    props: {
        ...(await serverSideTranslations(locale, ['common', 'navbar', 'reward-center', 'identification', 'payment-method']))
    }
});
