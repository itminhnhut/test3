...
const FaqTopics = (props) => {
    ...
    const cats = SupportCategories.faq[language]?.find(o => o.displaySlug === router?.query?.topic)?.subCats || false
    ...
    const renderGroupArticles = useCallback(() => {
        ...
        const data = articles?.filter(e => {
            const isBelongThisGroup = e?.tags?.find(o => o.slug === `faq-${language}-${currentGroup}`)
